...
def orig_mean_and_std_for_zscore_h5(spca_results, orig_data, spca_zht_params, pev_threshold=0.01):
    Z_list = []
    for ds in orig_data:
        data = orig_data[ds]
        
        _, _, _, indices = spca_zht_params[ds]
        d = data[:, indices]
        above_thresh = spca_results[ds]["pev"] >= pev_threshold
        Z = d.dot(spca_results[ds]["loadings"][:, above_thresh])
        if np.any(np.isnan(Z)):
            print("NaNs found", ds)
        Z_list.append(Z)
...
def spca_transform_new_data_h5(spca_results, new_data, spca_zht_params, orig_mean, orig_std, pev_threshold=0.01):
    """Transform and z-score new data by previously-determined components"""
    Z_list = []
    for ds in new_data:
        data = new_data[ds]
